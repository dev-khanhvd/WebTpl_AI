...
@router.post("/")
async def order_page_options():
    """Returns available options for the category page agent"""
    options = [
        {'id': 1, 'name': 'Lịch sử mua hàng'}
    ]

    return {"status": "success", "options": options}

@router.post("/process")
async def process_order_menu(request: OrderRequest):
    """Process the order page with the selected option"""
    ...
    checkout_page = OrderPage(folder_name)
    ...
    try:
        ...
        result = checkout_page.get_order_page_content(
            wrapper_classes,
            request.menu_choice,
            item_classes,
            request.options,
        ...
        return {"status": "success", "message": f"Processed order page option {request.menu_choice}",
                "result": result}
...
class OrderPage:
    def __init__(self, base_dir, base_branch=BASE_BRANCH):
        ...
        self.file_path = normalize_github_path(
            self.base_dir + "/" + self.template_mapping['order_index'])
    ...
    def get_order_page_content(self, wrapper_classes: str, choice_selected: int,item_classes=None,options=None):
