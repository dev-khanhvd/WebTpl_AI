...
@router.post("/process")
async def process_cart_menu(request: CartRequest):
    """Process the cart page with the selected option"""
    ...
    cart_page = CartPage(folder_name)

    if not cart_page.base_dir:
    ...
    try:
        ...
        result = cart_page.get_cart_page_content(
            wrapper_classes,
            request.menu_choice,
            item_classes,
            request.options,
        ...
        return {"status": "success", "message": f"Processed cart page option {request.menu_choice}",
                "result": result}
...
class CartPage:
    def __init__(self, base_dir, base_branch=BASE_BRANCH):
        ...
        self.file_path = normalize_github_path(
            self.base_dir + "/" + self.template_mapping['order_cart'])
    ...
    def get_cart_page_content(self, wrapper_classes: str, choice_selected: int,item_classes=None,options=None):
        ...
        try:
            ...
            match choice_selected:
                case 1:
                    question = "Trang giỏ hàng là nơi hiển thị các sản phẩm mà người dùng đã chọn mua"
